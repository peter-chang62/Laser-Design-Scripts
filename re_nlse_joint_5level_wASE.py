...
class Mode(pynlo.media.Mode):
    def __init__(
        ...
        assert isinstance(record_sim, bool)
        ...
        self._P_ASE = np.zeros(v_grid.size)
    ...
    def _dP_ASE_dz(self, z, P_ase):
        return (
            -self.sigma_a * self.n1
            + self.sigma_e * self.n2
            - self.sigma_a * self.eps_s * self.n2
        ) * self.overlap_s * P_ase + h * self.v_grid * self.dv * 2

    def _Pp_P_ASE_deriv(self, z, X):
        Pp = X[:2]
        P_ase = X[2:]
        dPp_dz = self._dPp_dz(z, Pp)
        dP_ASE_dz = self._dP_ASE_dz(z, P_ase)
        return np.hstack((dPp_dz, dP_ASE_dz))

    def setup_rk45_Pp(self, dz):
        self._rk45 = RK45(
            fun=self._Pp_P_ASE_deriv,
            t0=self._z_start,
            y0=np.hstack((np.array([self.Pp_fwd, self.Pp_bck]), self.P_ASE)),
            t_bound=np.inf,
            max_step=dz,
        )
    ...
    @property
    def rk45(self):
        assert self._rk45 is not None, "setup rk45 by calling setup_rk45_Pp(dz)"
        return self._rk45
    ...
    def propagate(self, a_v, z, z_stop, dz, local_error, k5_v=None, cont=False):
        ...
        while z < z_stop:
            ...
            if error_ratio > 2:
                ...
                if self.mode.record_sim:
                    self._sum_a_record.append(self.mode._sum_a_no_pre)
                    self._sum_e_record.append(self.mode._sum_e_no_pre)
